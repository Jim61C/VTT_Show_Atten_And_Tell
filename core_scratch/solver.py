...
class CaptioningSolver(object):
    ...
    def test(self, data, split='train', attention_visualization=False, save_sampled_captions=True, save_path='./data'):
        ...
        alphas, betas, sampled_captions, _, _, _, _ = self.model.build_vtt_model(max_len=17)    # (N, max_len, L), (N, max_len)
        ...
        with tf.Session(config=config) as sess:
            ...
            self.optimistic_restore(sess, self.test_model)
            ...
            if save_sampled_captions:
                all_sam_cap = np.ndarray((features.shape[0], 17))
        ...
        with tf.Session(config=config) as sess:
            ...
            for e in range(self.n_epochs):
                ...
                for i in range(n_iters_per_epoch):
                    ...
                    image_idxs_batch = image_idxs[i*self.batch_size:(i+1)*self.batch_size]
                    features_batch = features[image_idxs_batch]
                    feed_dict = {self.model.features: features_batch, self.model.captions: captions_batch, self.G_learning_rate: self.lr_G, self.D_learning_rate: self.lr_D, self.LSTM_learning_rate: self.lr_LSTM}
